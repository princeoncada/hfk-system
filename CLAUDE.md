# HFK Publishing Engine â€” Claude Code Session Rules
...
Grades 1â€“6 in Math, Science, Reading/Vocabulary, and Bible/Values.

Architecture invariants:
- content/ is the source of truth for all structured JSON content
- exports/ is a derived output layer â€” regenerable from content + templates
- archives/ is permanent output history â€” never deleted
- assets/avatars/ contains fixed brand assets â€” never regenerated by AI
- Templates control all visual output â€” never modified casually
- JSON controls all content â€” templates never embed content directly
...
- Provide 1-by-1 git commit command blocks for the user to run â€”
...
You do NOT (default â€” without the Implementation Gate unlock phrase):
...
Unlock phrase â€” required to authorize direct implementation:
...
- Stop â€” do not use Edit, Write, or any file-modification tool on
  project files
...
appear in one message â€” no AI turn between them:
- Validation summary
...
commit block â€” no separate turn:
```powershell
...
Codex writes or modifies files:
  ...
  - A ```powershell code block for the USER to copy, run, and paste
    back â€” Claude Code does NOT run these
  - Do NOT include npm install or any npm commands inside the Codex
    master prompt (SECTION 1) â€” npm commands belong here in Section 2
...
place â€” always open a new patch version.
...
States: alpha (implemented, not tested) â†’ beta (partial validation)
â†’ stable (fully validated, committed)

Four versioning locations â€” always update all four together:
...
- vault/           (creative input layer â€” brand, prompts, references)
- .env
