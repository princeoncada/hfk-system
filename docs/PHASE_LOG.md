...
| --- | --- | --- | --- | --- |
| 6.0.0-stable | Phase 6.0.0 | stable | 2026-05-25 | Template Lab: Short O Worksheet Prototype. Opens 6.x template production series. Closes 5.x workflow infrastructure series. Scaffold only: asset folders, placeholder SVGs, route stub, WorksheetCanvas stub, and asset library guide. |
...
- Bumped all five versioning locations to 6.0.0-stable and shifted active series from 5.x to 6.x.
...
Scope: CLAUDE.md, docs/WORKFLOW.md, docs/FUTURE_PLANS.md + five versioning files

Purpose: Harden query discipline and direction-confirmation protocol.
...
Changes:
- Updated session-start guidance to query ChromaDB before opening large docs, run exactly one query per topic, trust the first result, and state why fallback reads are needed.
- Updated New Phase workflow to require one hfk_docs query, present findings, and wait for explicit direction confirmation before scoping or Infrastructure Check.
- Updated docs/WORKFLOW.md with one-query-per-topic discipline, explicit phase-direction confirmation, and QUERY/CONFIRM steps in the standard phase cycle.
- Replaced the planned 5.5.0 Advanced Memory Layer with Workflow Hardening: Query Discipline and deferred Zep/Mem0 indefinitely at current scale.
- Bumped all five versioning locations to 5.5.0-stable.
...
Scope: Documentation-only hardening for phase scoping and implementation behavior.
...
Changes:
- Added Infrastructure Check rule to the New Phase workflow in CLAUDE.md.
...
- Bumped all five versioning locations to 5.2.7-stable.
